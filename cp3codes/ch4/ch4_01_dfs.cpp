...
int main() {
  ...
  */
  ...
  cout << "Standard DFS Demo (the input graph must be UNDIRECTED)" << endl;
  numCC = 0;
  ...
  cout << "Flood Fill Demo (the input graph must be UNDIRECTED)" << endl;
  numCC = 0;
  ...
  cout << "Topological Sort (the input graph must be DAG)" << endl;
  topoSort.clear();
  dfs_num.assign(V, DFS_WHITE);
  ...
  cout << "Articulation Points & Bridges (the input graph must be UNDIRECTED)" << endl;
  ...
  cout << "Strongly Connected Components (the input graph must be DIRECTED)" << endl;
